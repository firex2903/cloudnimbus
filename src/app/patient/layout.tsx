...
import { getSession, clearSession } from "@/lib/auth-store";
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-slate-100 shadow-sm">
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center gap-6">
            ...
            {navItems.map(({ href, icon: Icon, label }) => (
              <Link key={href} href={href}
                className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition-colors ${
                  pathname === href ? "bg-sky-50 text-sky-600" : "text-slate-600 hover:bg-sky-50 hover:text-sky-600"
                }`}>
                <Icon size={16} />
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleLogout} className="p-2 hover:bg-slate-100 rounded-xl transition-colors text-slate-400 hover:text-red-500">
              <LogOut size={16} />
            </button>
          </div>
        </div>
